...
void printformat()
{
	printf("Usage : ./orientation <mode>(optional) <event> <interval>(optional)\n\n");
...
int main(int argc,char **argv)
{
	...
	if (argc != 2 && argc != 3 && argc!=4) {
		...
		if (result < 0) {
			printf("Can't start orientation SENSOR\n");
			printf("Error\n\n\n\n");
			return -1;
		}
		...
		while(1) {
			result = sf_get_data(handle, ORIENTATION_BASE_DATA_SET , &data);
			printf("Orientation [%lld] [%6.6f] [%6.6f] [%6.6f]\n\n", data.timestamp, data.values[0], data.values[1], data.values[2]);
			usleep(100000);
		}
		...
		if (result < 0) {
			printf("Can't disconnect Orientation sensor\n");
			printf("Error\n\n\n\n");
			return -1;
		}
	}
		...
		if (result < 0)
			printf("Can't register orientation\n");
