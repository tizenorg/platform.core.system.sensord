...
void printformat()
{
	printf("Usage : ./geomagnetic_rv <mode>(optional) <event> <interval>(optional)\n\n");
	...
	printf("The time interval should be entered based on the sampling frequencies supported by "
			"accelerometer and geomagnetic sensors driver on the device in ms. If no value for "
			"geomagnetic rotation vector sensor is entered, a default value will be used.\n");
}
...
int main(int argc,char **argv)
{
	...
	sensor_type_t type = GEOMAGNETIC_RV_SENSOR;
	...
	if (argc != 2 && argc != 3 && argc!=4) {
		...
		if (result < 0) {
			printf("Can't start Geomagnetic Rotation Vector Sensor\n");
			printf("Error\n\n\n\n");
			return -1;
		}
		...
		while(1) {
			result = sf_get_data(handle, GEOMAGNETIC_RV_EVENT_RAW_DATA_REPORT_ON_TIME , &data);
			printf("Geomagnetic Rotation Vector [%lld] [%6.6f] [%6.6f] [%6.6f]\n\n", data.timestamp,
		...
		if (result < 0) {
			printf("Can't disconnect Geomagnetic Rotation Vector Sensor\n");
			printf("Error\n\n\n\n");
			return -1;
		}
	}
		...
		if (result < 0)
			printf("Can't register Geomagnetic Rotation Vector Event\n");
