...
void callback_temp(unsigned int event_type, sensor_event_data_t *event, void *user_data)
{
	sensor_data_t *data = (sensor_data_t *)event->event_data;
...
int main(int argc,char **argv)
{
	...
	switch(type) {
		...
		break;
		case(TEMPERATURE_SENSOR):
			result = sf_register_event(handle, event, event_condition, callback_temp, NULL);
		break;
		case(PROXIMITY_SENSOR):
