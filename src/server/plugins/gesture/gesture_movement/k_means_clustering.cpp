...
//					float euclid_distance = sqrt(diff_x_pow + diff_y_pow + diff_z_pow); // To expand distance between clusters, do not use sqrt
...
void k_means_clustering::print_data(void)
{
	...
	for (m = 0; m < MCOUNT; m++) {
		for (n = 0; n < NCOUNT; n++) {
			_E("@@@@@ m_data[%2d][%2d] {%6.6f, %6.6f, %6.6f, ci(%6d), ed(%6.6f)}",
				m, n, m_data[m][n].accel.x, m_data[m][n].accel.y, m_data[m][n].accel.z, m_data[m][n].ci, m_data[m][n].ed);
		}
	}

	for (n = 0; n < NCOUNT; n++) {
		_E("@@@@@ m_template_base[%2d] {ci(%2d)}", n, m_template_base[n].ci);
	}
}
...
void k_means_clustering::decide_initial_means(void)
{
	...
	for (int m = 0; m < MCOUNT; m++) {
		for (int n = 0; n < NCOUNT; n++) {
			if ((n % NCOUNT) == 5 ) { // FIXME : for TEMPORAL test, make m_means(k=10 clusters) with middle value of each gesture
