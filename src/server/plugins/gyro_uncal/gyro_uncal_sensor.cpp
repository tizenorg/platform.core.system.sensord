...
#define SENSOR_NAME "GYRO_UNCALSCOPE_SENSOR"
#define SENSOR_TYPE_GYRO_UNCAL		"GYRO_UNCALSCOPE"

#define GYROSCOPE_ENABLED 0x01
#define GYRO_BIAS_ENABLED 0x02
#define GYRO_UNCAL_BIAS_ENABLED 3
...
, m_time(0)
{
	...
	register_supported_event(GYRO_UNCAL_RAW_DATA_EVENT);
...
{
	types.push_back(GYRO_UNCALSCOPE_SENSOR);
}
...
bool gyro_uncal_sensor::on_start(void)
{
	...
	m_fusion_sensor->register_supported_event(FUSION_GYRO_UNCAL_EVENT);
	m_fusion_sensor->register_supported_event(FUSION_GYRO_UNCAL_ENABLED);
	m_fusion_sensor->add_client(FUSION_GYRO_UNCAL_EVENT);
...
bool gyro_uncal_sensor::on_stop(void)
{
	...
	m_fusion_sensor->delete_client(FUSION_GYRO_UNCAL_EVENT);
	m_fusion_sensor->delete_interval((intptr_t)this, false);
	m_fusion_sensor->unregister_supported_event(FUSION_GYRO_UNCAL_EVENT);
	m_fusion_sensor->unregister_supported_event(FUSION_GYRO_UNCAL_ENABLED);
...
void gyro_uncal_sensor::synthesize(const sensor_event_t &event, vector<sensor_event_t> &outs)
{
	...
	if (event.event_type == FUSION_GYRO_UNCAL_EVENT) {
		diff_time = event.data.timestamp - m_time;
	...
	if (m_enable_gyro_uncal == GYRO_UNCAL_BIAS_ENABLED) {
		m_enable_gyro_uncal = 0;

		m_time = get_timestamp();
		gyro_uncal_event.sensor_id = get_id();
		gyro_uncal_event.event_type = GYRO_UNCAL_RAW_DATA_EVENT;
...
int gyro_uncal_sensor::get_sensor_data(const unsigned int event_type, sensor_data_t &data)
{
	...
	if (event_type != GYRO_UNCAL_RAW_DATA_EVENT)
		return -1;

	m_fusion_sensor->get_sensor_data(FUSION_GYRO_UNCAL_ENABLED, fusion_data);
