...
static bool check_privilege_by_sockfd(int sock_fd, const char *priv)
{
	...
	if (cynara_creds_socket_get_client(sock_fd, CLIENT_METHOD_SMACK, &smack) != CYNARA_API_SUCCESS ||
			cynara_creds_socket_get_user(sock_fd, USER_METHOD_UID, &uid) != CYNARA_API_SUCCESS ||
