...
{
	const float LBS_TO_UTESLA = 0.06f;
	const int chance = 3;
	int retry = 0;
...
bool geo_sensor_hal::is_sensorhub_supported(void)
{
	...
	fp = fopen("/sys/class/sensors/ssp_sensor/mag_poll_delay", "r");
			...
			if (strncasecmp(dir_entry->d_name ,"input", 5) == 0) {
				...
				if (hw_name == string(INPUT_NAME)) {
					...
					if (m_sensorhub_supported) {
						...
						m_polling_resource = string("/sys/class/sensors/ssp_sensor/mag_poll_delay");
					} else {
