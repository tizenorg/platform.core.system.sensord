...
bool tester_manager::run(int argc, char *argv[])
{
	...
	if (argc == TESTER_ARGC || is_number(argv[3]))
	...
	result = _tester->test(type, argc, argv);
...
/*
 * Implementation of tester_option
 */
bool tester_option::m_full_log;

void tester_option::show_full_log(bool show)
{
	m_full_log = show;
}

bool tester_option::full_log(void)
{
	return m_full_log;
}
...
void tester_bench::stop_all_testcase(void)
{
	instance().stop();
}
...
{
	tester_result fail(function, line, msg);
...
: m_name(name)
{
	tester_bench::register_testcase(this);
}
