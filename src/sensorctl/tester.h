...
{ \
	...
	tester_bench::push_failure(__FUNCTION__, __LINE__, text); \
}

#define PASS_TEXT(condition, text) \
{ \
	if (tester_option::full_log()) { \
		_I("[   PASS   ] "); \
		PRINT("%s(%d) -> %s(%s)\n", __FUNCTION__, __LINE__, text, #condition); \
	} \
}
...
{ \
	ASSERT_IF_TEXT(condition, tester_case::name().c_str()); \
}

#define EXPECT_IF(condition) \
{ \
	EXPECT_IF_TEXT(condition, tester_case::name().c_str()); \
}

#define TESTCASE(tc_name) \
class test_case_##tc_name : public tester_case { \
public: \
	test_case_##tc_name() \
	: tester_case(#tc_name) \
	{ \
		register_func(static_cast<tester_case::test_func>(&test_case_##tc_name::test)); \
	} \
	bool test(void); \
