...
#include <inv_sensor_fusion.h>
#include <sensor_plugin_loader.h>

#define SENSOR_NAME "Sensor Fusion"

inv_sensor_fusion::inv_sensor_fusion()
{
...
bool inv_sensor_fusion::init(void)
{
...
bool inv_sensor_fusion::on_start(void)
{
	AUTOLOCK(m_mutex);
...
bool inv_sensor_fusion::on_stop(void)
{
...
bool inv_sensor_fusion::add_interval(int client_id, unsigned int interval)
{
	AUTOLOCK(m_mutex);
...
bool inv_sensor_fusion::delete_interval(int client_id)
{
	AUTOLOCK(m_mutex);
...
bool inv_sensor_fusion::get_properties(sensor_properties_t &properties)
{
...
void inv_sensor_fusion::fuse(const sensor_event_t& event)
{
	return;
}

bool inv_sensor_fusion::get_rotation_matrix(arr33_t &rot)
{
	return true;
}

bool inv_sensor_fusion::get_attitude(float &x, float &y, float &z, float &w)
{
	return true;
}

bool inv_sensor_fusion::get_gyro_bias(float &x, float &y, float &z)
{
	return true;
}

bool inv_sensor_fusion::get_rotation_vector(float &x, float &y, float &z, float &w, float &heading_accuracy)
{
	return true;
}

bool inv_sensor_fusion::get_linear_acceleration(float &x, float &y, float &z)
{
	return true;
}

bool inv_sensor_fusion::get_gravity(float &x, float &y, float &z)
{
	return true;
}

bool inv_sensor_fusion::get_rotation_vector_6axis(float &x, float &y, float &z, float &w, float &heading_accuracy)
{
	return true;
}


bool inv_sensor_fusion::get_geomagnetic_rotation_vector(float &x, float &y, float &z, float &w)
{
	return true;
}


bool inv_sensor_fusion::get_orientation(float &azimuth, float &pitch, float &roll)
{
	return true;
}
...
{
	inv_sensor_fusion *inst;

	try {
		inst = new inv_sensor_fusion();
	} catch (int err) {
		ERR("inv_sensor_fusion class create fail , errno : %d , errstr : %s\n", err, strerror(err));
		return NULL;
	}
