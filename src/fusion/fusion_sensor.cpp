...
, m_time(0)
{
	...
	register_supported_event(FUSION_CALIBRATION_EVENT);
	m_enable_fusion = 0;
...
void fusion_sensor::synthesize(const sensor_event_t &event, vector<sensor_event_t> &outs)
{
	...
	if (m_enable_fusion == ORIENTATION_ENABLED && is_supported(FUSION_ORIENTATION_EVENT)) {
		...
		fusion_event.data.values[1] = euler_orientation.m_ang.m_vec[0];
		fusion_event.data.values[2] = euler_orientation.m_ang.m_vec[1];
		if (euler_orientation.m_ang.m_vec[2] >= 0)
			fusion_event.data.values[0] = euler_orientation.m_ang.m_vec[2];
		else
			fusion_event.data.values[0] = euler_orientation.m_ang.m_vec[2] + azimuth_offset;
	...
	if (m_enable_fusion == ROTATION_VECTOR_ENABLED && is_supported(FUSION_ROTATION_VECTOR_EVENT)) {
		...
		fusion_event.data.values[0] = quaternion_orientation.m_quat.m_vec[1];
		fusion_event.data.values[1] = quaternion_orientation.m_quat.m_vec[2];
		fusion_event.data.values[2] = quaternion_orientation.m_quat.m_vec[3];
		fusion_event.data.values[3] = quaternion_orientation.m_quat.m_vec[0];

		push(fusion_event);
	}

	if (m_enable_gaming_rv == GAMING_RV_ENABLED && is_supported(FUSION_GAMING_ROTATION_VECTOR_EVENT)) {
		sensor_event_t fusion_event;
		...
		fusion_event.data.timestamp = m_time;
		fusion_event.data.value_count = 4;
		fusion_event.data.values[0] = quaternion_gaming_rv.m_quat.m_vec[1];
		fusion_event.data.values[1] = quaternion_gaming_rv.m_quat.m_vec[2];
		fusion_event.data.values[2] = quaternion_gaming_rv.m_quat.m_vec[3];
		fusion_event.data.values[3] = quaternion_gaming_rv.m_quat.m_vec[0];

		push(fusion_event);
	}

	if (m_enable_geomagnetic_rv == GEOMAGNETIC_RV_ENABLED && is_supported(FUSION_GEOMAGNETIC_ROTATION_VECTOR_EVENT)) {
		sensor_event_t fusion_event;
		...
		fusion_event.data.timestamp = m_time;
		fusion_event.data.value_count = 4;
		fusion_event.data.values[0] = quaternion_geo_rv.m_quat.m_vec[1];
		fusion_event.data.values[1] = quaternion_geo_rv.m_quat.m_vec[2];
		fusion_event.data.values[2] = quaternion_geo_rv.m_quat.m_vec[3];
		fusion_event.data.values[3] = quaternion_geo_rv.m_quat.m_vec[0];
