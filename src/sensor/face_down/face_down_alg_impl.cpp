...
void face_down_alg_impl::shift_quaternions(void) {
	//Can be made much more efficient if needed using LinkedLists
	if (m_current_number_of_quat < (WINDOW_SIZE - 1)) {
...
int face_down_alg_impl::is_facing_down(void) {
	//Can be made much more efficient if needed by precomputing during insertion
...
int face_down_alg_impl::was_facing_up(void) {
	//Can be made much more efficient by precomputing during insertion and shifting
