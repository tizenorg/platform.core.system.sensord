...
void* sensor_plugin_loader::load_module(const char* path)
{
	...
	if (!handle) {
		DBG("Target file is %s , dlerror : %s\n", path, dlerror());
		return NULL;
	}
...
bool sensor_plugin_loader::load_plugins(const string& plugins_path)
{
	...
	DBG("sensor_plugin_load::load_plugins(\"%s\") is called!\n", plugins_path.c_str());
