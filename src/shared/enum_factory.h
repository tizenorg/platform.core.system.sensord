...
#define ENUM_SENSOR_DEPRECATED(name, assign) ENUM_SENSOR_VALUE(name, assign)

#define ENUM_CASE(name) case (name): return #name;
#define ENUM_CASE_VALUE(name, assign) ENUM_CASE(name)
#define ENUM_CASE_IGNORE(name, assign)

#define DECLARE_SENSOR_ENUM(ENUM_TYPE, ENUM_DEF) \
	typedef enum ENUM_TYPE { \
		ENUM_DEF(ENUM_SENSOR, ENUM_SENSOR_VALUE, ENUM_SENSOR_DEPRECATED) \
	} ENUM_TYPE;
...
#define DECLARE_SENSOR_ENUM_UTIL(ENUM_TYPE, ENUM_DEF) \
	const char *util_##ENUM_TYPE::get_string(ENUM_TYPE type) { \
		switch (type) { \
		ENUM_DEF(ENUM_CASE, ENUM_CASE_VALUE, ENUM_CASE_IGNORE) \
		} \
		return "UNKNOWN"; \
	}
