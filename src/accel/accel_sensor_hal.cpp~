...
bool accel_sensor_hal::enable_resource(bool enable)
{
	...
	temp = m_accel_dir + string(SCAN_EL_DIR) + string(CHANNEL_NAME_X) + string(ENABLE_SUFFIX);
	if (update_sysfs_num(temp.c_str(), enable_val) < 0)
		return false;

	temp = m_accel_dir + string(SCAN_EL_DIR) + string(CHANNEL_NAME_Y) + string(ENABLE_SUFFIX);
	if (update_sysfs_num(temp.c_str(), enable_val) < 0)
		return false;

	temp = m_accel_dir + string(SCAN_EL_DIR) + string(CHANNEL_NAME_Z) + string(ENABLE_SUFFIX);
	if (update_sysfs_num(temp.c_str(), enable_val) < 0)
		return false;

	temp = m_accel_dir + string(SCAN_EL_DIR) + string(CHANNEL_NAME_TIME) + string(ENABLE_SUFFIX);
	if (update_sysfs_num(temp.c_str(), enable_val) < 0)
		return false;
...
bool accel_sensor_hal::setup_trigger(char* trig_name, bool verify)
{
	...
	ret = update_sysfs_string(temp.c_str(), trig_name, verify);
	if (ret < 0)
	{
...
bool accel_sensor_hal::setup_buffer(int enable)
{
	string temp;
	int ret;
	temp = m_accel_dir + string(BUFFER_LEN);
	...
	temp = m_accel_dir + string(BUFFER_EN);
