...
, m_interval(POLL_1HZ_MS)
{
	...
	vector<unsigned int> supported_events = {
		ACCELEROMETER_EVENT_RAW_DATA,
		ACCELEROMETER_EVENT_UNPROCESSED_DATA,
	};
...
bool accel_sensor::process_event(void)
{
	...
	if (get_client_cnt(ACCELEROMETER_EVENT_UNPROCESSED_DATA)) {
		base_event.sensor_id = get_id();
		base_event.event_type = ACCELEROMETER_EVENT_UNPROCESSED_DATA;
		push(base_event);
	}

	if (get_client_cnt(ACCELEROMETER_EVENT_RAW_DATA)) {
		base_event.sensor_id = get_id();
		base_event.event_type = ACCELEROMETER_EVENT_RAW_DATA;
...
int accel_sensor::get_sensor_data(unsigned int type, sensor_data_t &data)
{
	...
	if (type == ACCELEROMETER_EVENT_RAW_DATA) {
		raw_to_base(data);
	} else {
