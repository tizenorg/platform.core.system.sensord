...
cmutex sensor_shared::m_shared_mutex;

sensor_shared::sensor_shared()
{
}
...
bool sensor_shared::init(void *data)
{
	return true;
}

bool sensor_shared::set_interval(unsigned long val)
{
	return true;
}

long sensor_shared::set_command(unsigned int cmd, long val)
{
	return -1;
}

int sensor_shared::send_sensorhub_data(const char* data, int data_len)
{
	return -1;
}

int sensor_shared::get_sensor_data(sensor_data_t &data)
{
	return -1;
}

int sensor_shared::get_sensor_data(sensorhub_data_t &data)
{
	return -1;
}

unsigned long long sensor_shared::get_timestamp(void)
{
	struct timespec t;
...
unsigned long long sensor_shared::get_timestamp(timeval *t)
{
	if (!t) {
		ERR("t is NULL");
		return 0;
...
bool sensor_shared::is_sensorhub_controlled(const string &key)
{
...
bool sensor_shared::get_node_info(const node_info_query &query, node_info &info)
{
	bool ret = false;
	int method;
	string device_num;
...
void sensor_shared::show_node_info(node_info &info)
{
...
bool sensor_shared::get_iio_node_info(const string& enable_node_name, const string& device_num, node_info &info)
{
...
bool sensor_shared::get_sensorhub_iio_node_info(const string &interval_node_name, const string& device_num, node_info &info)
{
	const string base_dir = string("/sys/bus/iio/devices/iio:device") + device_num + string("/");
...
bool sensor_shared::get_input_event_node_info(const string& device_num, node_info &info)
{
	string base_dir;
	string event_num;
...
bool sensor_shared::get_sensorhub_input_event_node_info(const string &interval_node_name, const string& device_num, node_info &info)
{
...
bool sensor_shared::set_node_value(const string &node_path, int value)
{
...
bool sensor_shared::set_node_value(const string &node_path, unsigned long long value)
{
	fstream node(node_path, fstream::out);
...
bool sensor_shared::get_node_value(const string &node_path, int &value)
{
...
bool sensor_shared::set_enable_node(const string &node_path, bool sensorhub_controlled, bool enable, int enable_bit)
{
...
bool sensor_shared::find_model_id(const string &sensor_type, string &model_id)
{
...
bool sensor_shared::get_event_num(const string &input_path, string &event_num)
{
...
bool sensor_shared::get_input_method(const string &key, int &method, string &device_num)
{
