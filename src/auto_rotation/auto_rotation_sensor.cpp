...
bool auto_rotation_sensor::init()
{
	...
	if (m_enable_fusion && GAMING_RV_ENABLED)
		m_gyro_sensor = sensor_plugin_loader::get_instance().get_sensor(GYROSCOPE_SENSOR);
	if (m_enable_fusion && GEOMAGNETIC_RV_ENABLED)
...
bool auto_rotation_sensor::on_start(void)
{
	...
	if (m_enable_fusion == TILT_ENABLED)
	...
	else if (m_enable_fusion == ORIENTATION_ENABLED)
